#!/bin/bash
# Sweep v3: Expert layers progressive unfreeze (n_last 4 → all 36), no LoRA.
# Hypothesis: action_proj-only (SAFE) has insufficient capacity; LoRA r=4 on last 2
# (sweep_v2 rank 1) may also be too tight. Try larger contiguous Expert subset FT.
# Trainer = sft_stage2_expert_lastN_full.py (joint CE + diffusion, grad_clip=0.5, wd=1e-4).
# Lower lr as more layers unfrozen to keep per-param drift bounded.
...
QLOG=$LOGS/ft_sweep_v3.log
CSV=$LOGS/ft_sweep_results.csv
log() { echo "[$(date +%H:%M:%S)] $*" | tee -a $QLOG; }

# train_recipe: variable extra args after the fixed ones
train_recipe() {
    local RECIPE=$1 TRAINER=$2 LR=$3 NTRAIN=$4 NEPOCH=$5 N_LAST=$6
    ...
    log "  TRAIN $RECIPE: $TRAINER lr=$LR ntrain=$NTRAIN epoch=$NEPOCH n_last=$N_LAST timeout=${TO}s"
    cd $SCRIPTS
    timeout $TO torchrun --nproc_per_node=8 --master_port=29507 $TRAINER \
...
EOF
    ...
    echo "$RECIPE,sft_stage2_expert_lastN_full.py,n_last_FT,workflow,workflow,workflow,$TOKEN_OK,$PDMS,$NCC,$DAC,$TTC,$EP,$C2" >> $CSV
...
run_recipe() {
    local RECIPE=$1 TRAINER=$2 LR=$3 NTRAIN=$4 NEPOCH=$5 N_LAST=$6
    log "=== $RECIPE ==="
    train_recipe "$RECIPE" "$TRAINER" "$LR" "$NTRAIN" "$NEPOCH" "$N_LAST"
    eval_recipe "$RECIPE"
}

log "=== FT recipe sweep_v3 START (Expert progressive unfreeze, no LoRA) ==="

# n_last=4 — moderate expansion from sweep_v2 rank 4 (n_last=2)
run_recipe "wfv3_exp_last4_full_lr2e6_500"   sft_stage2_expert_lastN_full.py  2e-6  500  1  4

# n_last=8 — bigger subset, halve lr to control drift
run_recipe "wfv3_exp_last8_full_lr1e6_500"   sft_stage2_expert_lastN_full.py  1e-6  500  1  8

# n_last=16 — large subset (~44% of Expert), very low lr
run_recipe "wfv3_exp_last16_full_lr5e7_500"  sft_stage2_expert_lastN_full.py  5e-7  500  1  16

# n_last=36 — full Expert FT, lowest lr to keep drift bounded
run_recipe "wfv3_exp_all36_full_lr1e7_500"   sft_stage2_expert_lastN_full.py  1e-7  500  1  36

log "=== FT recipe sweep_v3 DONE ==="
log "results: $CSV"
cat $CSV | tee -a $QLOG
