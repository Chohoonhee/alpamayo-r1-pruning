#!/bin/bash
# Sweep v7: NAVSIM trainval exploration after v6 full-set baseline.
# Covers: lr scan on full, 2-epoch sweep, SAFE+token combo, VLM+Expert LoRA in-distribution.
...
train_recipe() {
    ...
    cd $SCRIPTS
    timeout $TO torchrun --nproc_per_node=8 --master_port=29511 $TRAINER \
...
EOF
    ...
    echo "$RECIPE,navsim_explore,?,navsim,navsim,navsim,$TOKEN_OK,$PDMS,$NCC,$DAC,$TTC,$EP,$C2" >> $CSV
...
log "=== FT recipe sweep_v7 START (NAVSIM full-set lr scan / 2 epoch / LoRA) ==="

# Token lr scan with 10k subset (~25 min train + 30 min eval each)
run_recipe "v7_navsim_token_10k_lr5e2"  sft_stage2_token_only_navsim.py  5e-2  10000  1
run_recipe "v7_navsim_token_10k_lr5e3"  sft_stage2_token_only_navsim.py  5e-3  10000  1
run_recipe "v7_navsim_token_10k_lr1e1"  sft_stage2_token_only_navsim.py  1e-1  10000  1

# SAFE lr scan with 10k subset
run_recipe "v7_navsim_safe_10k_lr5e5"   sft_stage2_safe_navsim.py        5e-5   10000  1
run_recipe "v7_navsim_safe_10k_lr1e4"   sft_stage2_safe_navsim.py        1e-4   10000  1

# 2 epoch on best (token-only lr=1e-2, 5k samples)
run_recipe "v7_navsim_token_5k_lr1e2_2ep"  sft_stage2_token_only_navsim.py  1e-2  5000  2

log "=== FT recipe sweep_v7 DONE ==="
log "results: $CSV"
...
cat $LOGS/FINAL_SUMMARY.txt | tee -a $QLOG
