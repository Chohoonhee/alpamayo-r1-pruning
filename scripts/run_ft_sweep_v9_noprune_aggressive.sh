#!/bin/bash
# Sweep v9: NO PRUNE + aggressive FT recipes that haven't been tried in this regime.
# All on NAVSIM trainval full (52k), no prune, more capacity than v8.
...
train_recipe() {
    ...
    cd $SCRIPTS
    timeout $TO torchrun --nproc_per_node=8 --master_port=29513 $TRAINER \
...
EOF
    ...
    echo "$RECIPE,navsim_noprune_aggressive,?,navsim,navsim,navsim,$TOKEN_OK,$PDMS,$NCC,$DAC,$TTC,$EP,$C2" >> $CSV
...
log "=== FT recipe sweep_v9 START (NO PRUNE + capacity-rich FT, NAVSIM full 52k) ==="

# 1) SAFE + token combined — both action_proj and lm_head special rows trainable
run_recipe "v9_noprune_safetoken_52k_lr1e4"  sft_stage2_safe_plus_token_navsim.py  1e-4  52000  1

# 2) Expert-only LoRA full set
run_recipe "v9_noprune_exp_52k_lr5e5"  sft_stage2_expert_only_navsim.py  5e-5  52000  1

# 3) VLM+Expert LoRA — most capacity (288M trainable)
run_recipe "v9_noprune_vlmexp_52k_lr1e5"  sft_stage2_navsim.py  1e-5  52000  1

# 4) VLM+Expert LoRA with lower lr (avoid overfit risk)
run_recipe "v9_noprune_vlmexp_52k_lr5e6"  sft_stage2_navsim.py  5e-6  52000  1

# 5) Best v8 winner scaled to 2 epochs (placeholder — replace if v8 winner identified)
run_recipe "v9_noprune_safetoken_52k_lr1e4_2ep"  sft_stage2_safe_plus_token_navsim.py  1e-4  52000  2

log "=== FT recipe sweep_v9 DONE ==="
log "results: $CSV"
