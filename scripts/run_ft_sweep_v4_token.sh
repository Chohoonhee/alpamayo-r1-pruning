#!/bin/bash
# Sweep v4: token-only restoration.
# Trains ONLY lm_head rows for special tokens (<|cot_start|>, <|cot_end|>,
# <|traj_future_start|>, <|traj_future|>, <|traj_future_end|>).
# Hypothesis: trajectory distribution stays at baseline; FT only restores
# <|traj_future_start|> generation. Expected ceiling = zero-shot pruned ~0.64.
...
train_recipe() {
    ...
    cd $SCRIPTS
    timeout $TO torchrun --nproc_per_node=8 --master_port=29508 $TRAINER \
...
EOF
    ...
    echo "$RECIPE,sft_stage2_token_only.py,token_rows,workflow,workflow,workflow,$TOKEN_OK,$PDMS,$NCC,$DAC,$TTC,$EP,$C2" >> $CSV
...
log "=== FT recipe sweep_v4 START (token-only restoration) ==="

run_recipe "wfv4_token_lr1e3_500"  sft_stage2_token_only.py  1e-3  500  1
run_recipe "wfv4_token_lr1e4_500"  sft_stage2_token_only.py  1e-4  500  1
run_recipe "wfv4_token_lr1e2_500"  sft_stage2_token_only.py  1e-2  500  1

log "=== FT recipe sweep_v4 DONE ==="
log "results: $CSV"
cat $CSV | tee -a $QLOG
