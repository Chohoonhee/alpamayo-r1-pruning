#!/bin/bash
# Sweep v2: 5 NEW recipes from Workflow synthesis (wf_a4a22b82-e6d).
# All 16 originally proposed recipes were adversarially refuted. These 5 survived
# the second round, designed against the actual CSV evidence:
#   safe_lr1e5_1k = 0.500 (best), Expert-only lr scan all <0.36, vlmexp all <0.5
# Strategy: SAFE+Expert-last2 LoRA + masked-CE removing 128 <|traj_future|> placeholders.
...
train_recipe() {
    ...
    cd $SCRIPTS
    timeout $TO torchrun --nproc_per_node=8 --master_port=29506 $TRAINER \
...
EOF
    ...
    TRAIN_M=$(grep -E "trainable.*M.*total" $LOGS/ft_sweep_v2.log 2>/dev/null | tail -1 | sed -E 's/.*trainable ([0-9.]+)M.*/\1/')

    LR=$(grep "^${RECIPE}," $QLOG.tmp 2>/dev/null | head -1)
    echo "$RECIPE,$(basename $(grep "TRAIN $RECIPE" $QLOG | tail -1 | awk '{print $4}')),$TRAIN_M,workflow,workflow,workflow,$TOKEN_OK,$PDMS,$NCC,$DAC,$TTC,$EP,$C2" >> $CSV
...
log "=== FT recipe sweep_v2 START (Workflow-recommended 5 recipes) ==="

# Rank 1: SAFE + LoRA r=4 last-2 Expert layers + masked CE (drop 128 placeholders)
run_recipe "wfv2_safe_exp_last2_lr5e6_500"  sft_stage2_safe_plus_exp_last2.py    5e-6  500  1

# Rank 2: SAFE + masked-placeholder CE only (cheapest fix)
run_recipe "wfv2_safe_mask_lr1e5_500"       sft_stage2_safe_mask_placeholders.py 1e-5  500  1

# Rank 3: SAFE+mask earlystop bracket (saves multiple checkpoints)
run_recipe "wfv2_safe_earlystop_lr1e5_1k"   sft_stage2_safe_earlystop.py         1e-5  1000 1

# Rank 4: Full-weight FT of last 2 Expert layers (higher capacity than action_proj-only)
run_recipe "wfv2_exp_last2_full_lr2e6_500"  sft_stage2_expert_lastN_full.py      2e-6  500  1

# Rank 5: SAFE+mask wd safety net
run_recipe "wfv2_safe_mask_wd_lr5e6_1k"     sft_stage2_safe_mask_wd.py           5e-6  1000 1

log "=== FT recipe sweep_v2 DONE ==="
log "results: $CSV"
cat $CSV | tee -a $QLOG
