#!/bin/bash
# Sweep v6: NAVSIM trainval FULL set (52k samples, stride=5).
# Test: does scale (sample count) matter, or is it always the floor?
# Two recipes: best token-only + best SAFE, both 1 epoch on full set.
...
train_recipe() {
    ...
    local TO=$((TOTAL_SAMPLES * 4 + 1800))   # full set ~10h headroom
    log "  TRAIN $RECIPE: $TRAINER lr=$LR ntrain=$NTRAIN epoch=$NEPOCH timeout=${TO}s"
    cd $SCRIPTS
    timeout $TO torchrun --nproc_per_node=8 --master_port=29510 $TRAINER \
        --drop_layers_json $POLICY \
        --train_samples $NTRAIN --epochs $NEPOCH \
        --lr $LR --batch_size 1 --grad_accum 8 \
        --out_dir $OUT --ddp \
        --log_every 20 --save_every 200 2>&1 | tail -80 | tee -a $QLOG
    sleep 5
}
...
EOF
    ...
    echo "$RECIPE,navsim_ft_full,?,navsim,navsim,navsim,$TOKEN_OK,$PDMS,$NCC,$DAC,$TTC,$EP,$C2" >> $CSV
...
log "=== FT recipe sweep_v6 START (NAVSIM trainval FULL ~52k samples) ==="

# Best recipe (token-only lr=1e-2) on full NAVSIM trainval
run_recipe "v6_navsim_token_full_lr1e2"  sft_stage2_token_only_navsim.py  1e-2  52000  1

# Best SAFE recipe on full NAVSIM trainval
run_recipe "v6_navsim_safe_full_lr1e5"   sft_stage2_safe_navsim.py        1e-5  52000  1

log "=== FT recipe sweep_v6 DONE ==="
log "results: $CSV"
cat $CSV | tee -a $QLOG
