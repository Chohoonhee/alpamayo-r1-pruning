...
class NavsimSFTDataset(Dataset):
    def __init__(
        ...
        INDEX_CACHE = os.path.join(os.path.dirname(__file__), "navsim_trainval_index.json"); import json as _json
        # Load cached index (built once by build_index.py) to avoid loading all pkls
        import json
        index_cache = os.path.join(os.path.dirname(os.path.abspath(__file__)), "navsim_trainval_index.json")
